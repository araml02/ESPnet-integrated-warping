...
class ESPnetASRModel(AbsESPnetModel):
    ...
    def warp_init():
        ...
        # self.warp_layer = PiecewiseLinearVTLNWarp(predictor=True, single_alpha=True)
        # self.warp_layer = PiecewiseLinearVTLNWarp(predictor=False, single_alpha=False)
        for param in self.warp_layer.parameters():
            param.requires_grad = True

        print("Disabling Model Parameters ... ")
        modules_to_freeze = [
            frontend, specaug, normalize,
            preencoder, encoder, postencoder,
            decoder, ctc, joint_network
        ]
        for module in modules_to_freeze:
            if module is not None:
                # freeze_module(module)
                # module.eval() # don't put in eval cause this causes batch size 1
                total = sum(p.numel() for p in module.parameters())
                trainable = sum(p.numel() for p in module.parameters() if p.requires_grad)
                print(f"{module.__class__.__name__}: total={total}, trainable={trainable}")
                
                for param in module.parameters():
                    param.requires_grad = False
        # Very important to disable the encoder normalizer
        for i, block in enumerate(encoder.encoders):
            if hasattr(block, "conv_module") and hasattr(block.conv_module, "norm"):
                bn = block.conv_module.norm
                if isinstance(bn, torch.nn.BatchNorm1d):
                    bn.eval()                     # use running stats instead of batch stats
                    bn.track_running_stats = False  # <- critical: prevents running_mean/var updates
                    for param in bn.parameters():
                        param.requires_grad = False  # optional: if you want to freeze its weights
                print(f"[Freeze] encoder.encoders frozen ({bn.__class__.__name__})")
    ...
    def forward(
        ...
        **kwargs,
        ...
        assert isinstance(encoder_out, torch.Tensor), f"Expected tensor, got {type(encoder_out)}"
