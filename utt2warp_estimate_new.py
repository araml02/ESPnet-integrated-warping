...
def get_reference_trn(ref_text, utt_id):
    ...
    # ref_txt = ref_text.replace(" ", "<space>")
    # ref_text = f"{ref_text.strip()} ({utt_id})" #reftrn
    ref_text = ref_text.strip()
    return ref_text

def compute_sclite_score(ref_trn, hyp_trn):
    with tempfile.TemporaryDirectory() as tmpdir:
        ref_path = os.path.join(tmpdir, "ref.trn")
        hyp_path = os.path.join(tmpdir, "hyp.trn")
        ...
        cmd = f"sclite -r {ref_path} trn -h {hyp_path} trn -i rm -o all stdout > {result_path}"
        subprocess.run(cmd, shell=True, check=True)

        with open(result_path, "r") as f:
            lines = f.readlines()
            for i, line in enumerate(lines):
                if "Err" in line and "SPKR" in line:
                    if i + 2 < len(lines):
                        data_line = lines[i + 2]
                        parts = data_line.split("|")
                        # print(parts)
                        if len(parts) >= 4:
                            # The score fields are in the third part
                            values = parts[3].split()
                            # print(values)
                            if len(values) >= 4 and '*' not in values[4]:
                                return float(values[4])  # 'Err' percentage
        return float("inf")
...
def run_speaker_grid_search(decode, wordtask, speech2text,spk, utt_list, scp_reader, text, model, encode_text, device, max_dur, alpha_range=(0.75, 1.20), num_points=10):
...
def main():
    ...
    model.to(device)
    if not hasattr(model, "warp_layer"):
    ...
    for spk, utt_list in spk2utt.items():
        ...
        if grid_search:
            start_value = run_speaker_grid_search(decode, wordtask, speech2text,spk, utt_list, scp_reader, text, model, encode_text, device, max_dur_per_speaker, alpha_range=alpha_range, num_points=num_gs_points)
            start = start_value
        else:
            start = 1.0
        alpha_raw = torch.nn.Parameter(torch.tensor(start, device=device))
        # model.warp_layer.register_parameter("alpha1_raw", alpha_raw)
        ...
        best_alpha = 1.0 if not grid_search else start_value
        best_loss = float("inf")
