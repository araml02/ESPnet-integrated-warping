...
class Speech2Text:
    ...
    @torch.no_grad()
    @typechecked
    def __call__(self, speech: Union[torch.Tensor, np.ndarray], warp: Optional[torch.Tensor] = None) -> Union[
        ListOfHypothesis,
        ...
        for keys, batch in loader:
            ...
            if batch.get("warp"):
                batch = {
                    "speech": batch["speech"],
                    "warp": batch["warp"],
                } # Maybe this should have been batch["speech"][0] bcs batch size always 1
            # ======= ARAM ==================
