...
class ESPnetDataset(AbsDataset):
    ...
    def __init__(
        ...
        for path, name, _type in path_name_type_list:
            ...
            # ARAM ===============
            if "utt2warp_ref" in path:
                utt2warp_path = os.path.join(os.path.dirname(path), "utt2warp_ref")
                if os.path.exists(utt2warp_path):
                    print(f"utt2warp exists at: {utt2warp_path}")
                    with open(utt2warp_path, "r") as f:
                        for line in f:
                            utt_id, warp_factor = line.strip().split()
                            self.utt2warp[utt_id] = float(warp_factor)
                continue # Make sure to continue if we don't have utt2warp_ref, otherwise error
                # TODO: add
            else:
                print(path)
            # ARAM ===============
    ...
    def __getitem__(self, uid: Union[str, int]) -> Tuple[str, Dict[str, np.ndarray]]:
        ...
        # Aram ================
        # data["warp"] = np.array([1.1], dtype=np.float32)
        if self.utt2warp is not None:
            if uid in self.utt2warp:
                # data["warp"] = np.array([[self.utt2warp[uid]]], dtype=np.float32)
                data["warp"] = np.array([self.utt2warp[uid]], dtype=np.float32)  # <-- shape: [1]

            else:
                # data["warp"] = np.array([[0.0]], dtype=np.float32)
                data["warp"] = np.array([0.0], dtype=np.float32)  # <-- shape: [1]
